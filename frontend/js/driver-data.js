(function () {
    ...
    const backgroundVideo = document.getElementById("driverDataBackgroundVideo");
    ...
    function setContextMeta(championshipName, championshipId) {
        if (championshipName) {
            contextMeta.textContent = `Showing drivers for ${championshipName}.`;
            return;
        }

        contextMeta.textContent = `Showing drivers for championship #${championshipId}.`;
    }
    ...
    function renderRows(rows) {
        ...
        if (!Array.isArray(rows) || rows.length === 0) {
            rowsEl.innerHTML = '<tr><td class="muted-cell" colspan="5">No driver rows found in SQL for this championship.</td></tr>';
            return;
        }
    ...
    async function loadDriverData() {
        try {
            ...
            setStatus("Loading driver rows from SQL database...", false);

            const response = await fetch(`/api/championships/${championshipId}/drivers`);
            if (!response.ok) {
                let details = "";
            ...
            renderRows(rows);
            setStatus(`Loaded ${rows.length} driver row${rows.length === 1 ? "" : "s"} from SQL.`, false);
        } catch (error) {
            renderRows([]);
            setStatus(`Unable to load SQL driver table. ${error.message}`, true);
        }
    }
